...
async function ensurePnpmAvailable(): Promise<void> {
  try {
    await execa("pnpm", ["--version"]);
  } catch {
    throw new Error(
      "pnpm is not available. Install it from https://pnpm.io/installation",
    );
  }
}

function ensureProjectDirAvailable(projectDir: string): void {
...
async function run(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (features.includes("convex")) {
        agentSections.push(
          "- Convex queries = reads",
          "- Convex mutations = writes",
        );
      }
      ...
      if (features.includes("ai")) {
        ...
        await fs.writeFile(
          path.join(projectDir, "src/lib/ai/client.ts"),
